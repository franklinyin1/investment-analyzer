  ...
  render() {
    ...
    if (company.financials) {
      ...
      let financials = company.financials.filter((financial) => {
        let presentation = company.presentations.filter((presentation) => {
          return presentation.adsh === financial.adsh && presentation.stmt === 'UN'
        })
      ...
      currentQuarterFinancials = currentQuarterFinancials.map(financial => {
        let presentation = company.presentations.filter((presentation) => {
          return presentation.adsh === financial.adsh && presentation.stmt === 'UN' && presentation.tag === financial.tag
        })
    ...
    return (
      <React.Fragment>
        {rows.length > 1 ? (
          <React.Fragment>
            <h3>Unclassifiable Statement</h3>
